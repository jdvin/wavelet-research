...
class RelativePositionTransformer(nn.Module):
    ...
    def from_pretrained(
        ...
        d_n = cls(
            ...
            cross_attn=cross_attn,
            is_causal=is_causal,
            use_geglu=config.text_encoder_use_geglu,
        )
...
class TextEncoder(RelativePositionTransformer):
    ...
    @classmethod
    def from_pretrained(
        cls, source_model: nn.Module, config: TelepathConfig, *args, **kwargs
    ):
        return super().from_pretrained(
            source_model, config, cross_attn=False, is_causal=False
        )
...
class TextDecoder(RelativePositionTransformer):
    ...
    def from_pretrained(
        ...
        return super().from_pretrained(
            source_model, config, cross_attn=True, is_causal=True
        )
...
class TelepathTrainer(nn.Module):
    ...
    def step(self, batch: dict[str, Tensor]) -> tuple[Tensor, Tensor, Tensor]:
        ...
        logits = torch.empty(B, B * self.world_size, device=self.device)
        ...
        for i in range(self.world_size):
            ...
            logits[:, i * B : i * B + B] = logits_i.clone()
        all_logits = torch.empty_like(labels, device=self.device, dtype=logits.dtype)
        all_gather_into_tensor(all_logits, logits)
